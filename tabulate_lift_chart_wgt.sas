...
%macro tabulate_lift_chart_wgt(in_lib=work,in_table=,score=,the_var=,wgt=,
...
/*   Macro generates WEIGHTED one-way lift chart of UNIT INCIDENCE						*/
...
proc report data = step4 nowindows headline headskip missing out = unwgted; 
	...
	define &the_var / analysis mean format=percent8.2 width =9 'Average of Dependent' noprint;
...
data wgted;
	set wgted (keep = bin &score &score._max &score._mean &the_var &the_var._sum &the_var._pct);
...
proc report data = wgt_lift nowindows headline headskip missing out = &out_table; 
	...
	define &wgt / analysis sum format=comma15. width=18 'Estimated Population' NOPRINT;
	define wgt_pct / analysis sum format = percent8.2 width=12 'Estimated Percent of Population' NOPRINT;
	
	define est_pop / analysis sum format=comma15. width=18 'Estimated Population';
	...
	define &score / analysis min format=6.2  width = 9 "Minimum &Score";
	define &score._max / analysis max format=6.2 width = 9 "Maximum &Score";
	define &score._mean / analysis mean format=comma5.2 width=12 "Average &Score";
	define &the_var / analysis mean format=percent8.2 width =12 "Average of &the_var";
	define &the_var._sum / analysiS Sum format=comma9. width=12 "Sum of &the_var";
	define &the_var._pct / analysis sum format=percent6.2 width = 12 "Percent of &the_var";
	define cum_pct / analysis max format=percent6.2 width = 12 "Cumulative Percent of &the_var";
	define cum_lift / analysis min format=8.2 width = 12 'Cumulative Lift';
	define non_cum_lift / analysis mean format=8.2 width = 14 'Non-Cumulative Lift';

	title "Weighted Lift Chart Using %upcase(&score) Score in Predicting %upcase(&the_var) Response Using Weight=%upcase(&wgt)";

	*rbreak after / dol dul summarize;

run;

%mend tabulate_lift_chart_wgt;
